...
+ (NSUInteger)differFirstDate:(NSDate *)date1
    ...
    NSDateComponents *comps = [gregorian components:unit fromDate:date2  toDate:date1  options:0];
    NSInteger value = 0;
    switch (ditterType) {
        case YDDifferDateTypeSecond:
            value = [comps second];
            break;
        case YDDifferDateTypeMinute:
            value = [comps minute];
            break;
        case YDDifferDateTypeHour:
            value = [comps hour];
            break;
        case YDDifferDateTypeDay:
            value = [comps day];
            break;
        case YDDifferDateTypemonth:
            value = [comps month];
            break;
        case YDDifferDateTypeYear:
            value = [comps year];
            break;
        default:
            break;
...
+ (NSDate *)dateFromTimeStamp:(id)timeStamp{
    ...
    if ([timeStamp isKindOfClass:[NSString class]]) {
        return [NSDate dateWithTimeIntervalSince1970:[(NSString *)timeStamp integerValue]];
    }
    else if ([timeStamp isKindOfClass:[NSNumber class]]){
        return [NSDate dateWithTimeIntervalSince1970:[(NSNumber *)timeStamp integerValue]];
    }
...
+ (NSString *)timeInfoWithDate:(id)unknownDate{
    NSDate *date = [NSDate dateFromTimeStamp:unknownDate];
    if (date == nil) {
        return @"";
    }
    ...
    if (time < 3600) { // 小于一小时
        retTime = time / 60;
        retTime = retTime <= 1.0 ? 1.0 : retTime;
        return [NSString stringWithFormat:@"%ld分钟前",(NSInteger)retTime];
    }
    else if (time < 3600 * 24) { // 小于一天，也就是今天
        retTime = time / 3600;
        retTime = retTime <= 1.0 ? 1.0 : retTime;
        return [NSString stringWithFormat:@"%ld小时前",(NSInteger)retTime];
    }
    else if (time < 3600 * 24 * 2) {// 大于一天
        return [NSString stringWithFormat:@"昨天 %02ld:%02ld",date.hour,date.minute];
    }
    else{
        return [NSString stringWithFormat:@"%ld-%02ld-%02ld %02ld:%02ld",date.year,date.month,date.day,date.hour,date.minute];
    }
}
...
+ (NSString *)yd_timeInformationFromyyyyMMddHHmmss:(NSString *)ymdhsString{
    ...
    NSDate *date = [dateFor dateFromString:ymdhsString];
    
    return [NSDate yd_timeInformationFromTimestamp:[date timeStampFromDate]];
}

+ (NSString *)formatYear_Month_Day:(id)time{
    NSDate *date = [NSDate dateFromTimeStamp:time];
...
+ (NSString *)format_yyyy_mm_dd_hh_mm_ss:(NSNumber *)timeStamp{
    ...
    return [NSString stringWithFormat:@"%ld-%02ld-%02ld %02ld:%02ld:%02ld",date.year,date.month,date.day,date.hour,date.minute,date.second];
}
...
+ (NSString *)yd_timeInformationFromTimestamp:(NSString *)timeStamp{
    ...
    return [NSDate yd_timeInformationFromDate:[NSDate dateFromTimeStamp:timeStamp]];
}
