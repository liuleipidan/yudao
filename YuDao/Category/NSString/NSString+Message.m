...
- (NSAttributedString *)toMessageString;
{
    ...
    if (!re) {
        YDLog(@"[NSString toMessageString]: %@", [error localizedDescription]);
    ...
    for(NSTextCheckingResult *match in resultArray) {
        ...
        YDEmojiGroup *group = [[YDEmojiKBHelper sharedInstance] defaultFaceGroup];
        for (YDEmoji *emoji in group.data) {
            if ([emoji.emojiName isEqualToString:subStr]) {
                //face[i][@"png"]就是我们要加载的图片
                //新建文字附件来存放我们的图片,iOS7才新加的对象
                NSTextAttachment *textAttachment = [[NSTextAttachment alloc] init];
                //给附件添加图片
                textAttachment.image = [UIImage imageNamed:emoji.emojiPath];
