...
@interface YDPopupController () <UIGestureRecognizerDelegate>
...
@implementation YDPopupController
...
- (instancetype)initWithContents:(NSArray <UIView *> *)contents {
    ...
    if (self) {
        ...
        self.theme = [YDPopupTheme defaultTheme];
        ...
        [YDNotificationCenter addObserver:self
...
- (void)dealloc {
    ...
    [YDNotificationCenter removeObserver:self name:UIApplicationWillChangeStatusBarOrientationNotification object:nil];
    [YDNotificationCenter removeObserver:self name:UIApplicationDidChangeStatusBarOrientationNotification object:nil];
...
- (void)applyTheme {
    if (self.theme.popupStyle == YDPopupStyleFullscreen) {
        self.theme.presentationStyle = YDPopupPresentationStyleFadeIn;
    }
    if (self.theme.popupStyle == YDPopupStyleActionSheet) {
        self.theme.presentationStyle = YDPopupPresentationStyleSlideInFromBottom;
    }
    self.blurEffectView.alpha = self.theme.blurEffectAlpha;
    self.popupView.layer.cornerRadius = self.theme.popupStyle == YDPopupStyleCentered?self.theme.cornerRadius:0;
    ...
    if (self.theme.popupStyle == YDPopupStyleFullscreen) {
        ...
        switch (self.theme.maskType) {
            case YDPopupMaskTypeClear:
                maskBackgroundColor = [UIColor clearColor];
                break;
            case YDPopupMaskTypeDimmed:
                maskBackgroundColor = [UIColor colorWithWhite:0.0 alpha:0.4];
                break;
            default:
...
- (void)dismissPopupControllerAnimated:(BOOL)flag {
    ...
    [self dismissPopupControllerAnimated:flag isTapBackgroud:NO];
}

- (void)dismissPopupControllerAnimated:(BOOL)flag isTapBackgroud:(BOOL)isTapBackgroud{
    ...
    [UIView animateWithDuration:flag?self.theme.animationDuration:0.0 animations:^{
        ...
        if ([self.delegate respondsToSelector:@selector(popupControllerDidDismiss:isTapBackgroud:)]) {
            [self.delegate popupControllerDidDismiss:self isTapBackgroud:isTapBackgroud];
        }
    }];
}
...
- (CGFloat)popupWidth {
    ...
    if (width > maskViewWidth || self.theme.popupStyle == YDPopupStyleFullscreen) {
        width = maskViewWidth;
    }
    return width;
}
...
- (void)handleBackgroundTapGesture:(id)sender {
    if (self.theme.shouldDismissOnBackgroundTouch) {
        ...
        [self dismissPopupControllerAnimated:self.dismissAnimated isTapBackgroud:YES];
    }
}
...
#pragma mark - YDPopupButton Methods

@implementation YDPopupButton
...
#pragma mark - YDPopupTheme Methods

@implementation YDPopupTheme

+ (YDPopupTheme *)defaultTheme {
    YDPopupTheme *defaultTheme = [[YDPopupTheme alloc] init];
    defaultTheme.backgroundColor = [UIColor whiteColor];
    defaultTheme.cornerRadius = 4.0f;
    defaultTheme.popupContentInsets = UIEdgeInsetsZero;
    defaultTheme.popupStyle = YDPopupStyleActionSheet;
    defaultTheme.presentationStyle = YDPopupPresentationStyleSlideInFromBottom;
    defaultTheme.dismissesOppositeDirection = NO;
    defaultTheme.maskType = YDPopupMaskTypeDimmed;
