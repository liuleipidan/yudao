...
+ (instancetype)manager {
    ...
    dispatch_once(&onceToken, ^{
        ...
        TZSCREEN_WIDTH = [UIScreen mainScreen].bounds.size.width;
        // 测试发现，如果scale在plus真机上取到3.0，内存会增大特别多。故这里写死成2.0
        TZScreenScale = 2.0;
        if (TZSCREEN_WIDTH > 700) {
            TZScreenScale = 1.5;
        }
    });
    return manager;
...
- (void)setColumnNumber:(NSInteger)columnNumber {
    ...
    CGFloat margin = 4;
    CGFloat itemWH = (TZSCREEN_WIDTH - 2 * margin - 4) / columnNumber - margin;
...
- (PHImageRequestID)getPhotoWithAsset:(id)asset completion:(void (^)(UIImage *, NSDictionary *, BOOL isDegraded))completion {
    CGFloat fullSCREEN_WIDTH = TZSCREEN_WIDTH;
    if (fullSCREEN_WIDTH > _photoPreviewMaxWidth) {
        fullSCREEN_WIDTH = _photoPreviewMaxWidth;
    }
    return [self getPhotoWithAsset:asset photoWidth:fullSCREEN_WIDTH completion:completion progressHandler:nil networkAccessAllowed:YES];
}
...
- (PHImageRequestID)getPhotoWithAsset:(id)asset completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    ...
    return [self getPhotoWithAsset:asset photoWidth:fullSCREEN_WIDTH completion:completion progressHandler:progressHandler networkAccessAllowed:networkAccessAllowed];
}
...
- (PHImageRequestID)getPhotoWithAsset:(id)asset photoWidth:(CGFloat)photoWidth completion:(void (^)(UIImage *photo,NSDictionary *info,BOOL isDegraded))completion progressHandler:(void (^)(double progress, NSError *error, BOOL *stop, NSDictionary *info))progressHandler networkAccessAllowed:(BOOL)networkAccessAllowed {
    if ([asset isKindOfClass:[PHAsset class]]) {
        CGSize imageSize;
        if (photoWidth < TZSCREEN_WIDTH && photoWidth < _photoPreviewMaxWidth) {
