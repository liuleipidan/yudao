...
@interface YDScrollMenuView ()

@property (nonatomic, strong) UIView *indicatorView;

@end

@implementation YDScrollMenuView
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        // default
        self.indicatorWidth = 37.0f;
        self.menuViewWidth = 45.f;
        self.menuViewMargin = 0.0f;
        self.indicatorHeight = 3.0f;
        
        _viewbackgroudColor = [UIColor whiteColor];
        _itemfont = [UIFont systemFontOfSize:16];
        _itemTitleColor = [UIColor orangeTextColor];
        _itemSelectedTitleColor = [UIColor orangeTextColor];
        _itemIndicatorColor = [UIColor orangeTextColor];
...
- (void)setItemTitleArray:(NSArray *)itemTitleArray
{
    if (_itemTitleArray != itemTitleArray) {
        self.menuViewWidth = self.frame.size.width/itemTitleArray.count;
        NSString *firstStr = itemTitleArray.firstObject;
        if (firstStr.length == 2) {
            self.indicatorWidth = 37;
        }
        if (firstStr.length > 2) {
        ...
        for (int i = 0; i < itemTitleArray.count; i++) {
            CGRect frame = CGRectMake(i*self.menuViewWidth, 0, self.menuViewWidth, CGRectGetHeight(self.frame));
        ...
        _indicatorView.frame = CGRectMake((self.menuViewWidth-self.indicatorWidth)/2, _scrollView.frame.size.height - self.indicatorHeight, self.indicatorWidth, self.indicatorHeight);
...
- (void)setIndicatorViewFrameWithRatio:(CGFloat)ratio isNextItem:(BOOL)isNextItem toIndex:(NSInteger)toIndex
{
    ...
    if (isNextItem) {
        indicatorX = ((self.menuViewMargin + self.menuViewWidth) * ratio ) + (toIndex * self.menuViewWidth) + ((toIndex + 1) * self.menuViewMargin) + (self.menuViewWidth-self.indicatorWidth)/2;
    } else {
        indicatorX =  ((self.menuViewMargin + self.menuViewWidth) * (1 - ratio) ) + (toIndex * self.menuViewWidth) + ((toIndex + 1) * self.menuViewMargin) + (self.menuViewWidth-self.indicatorWidth)/2;
    }
    
    if (indicatorX < self.menuViewMargin || indicatorX > self.scrollView.contentSize.width - (self.menuViewMargin + self.menuViewWidth) + (self.menuViewWidth-self.indicatorWidth)/2) {
        return;
    }
    _indicatorView.frame = CGRectMake(indicatorX, _scrollView.frame.size.height - self.indicatorHeight, self.indicatorWidth, self.indicatorHeight);
