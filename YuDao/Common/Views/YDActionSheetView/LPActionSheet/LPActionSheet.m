...
- (instancetype)initWithTitle:(NSString *)title cancelButtonTitle:(NSString *)cancelButtonTitle destructiveButtonTitle:(NSString *)destructiveButtonTitle otherButtonTitles:(NSArray *)otherButtonTitles handler:(LPActionSheetBlock)actionSheetBlock
{
    ...
    if (self)
    {
        ...
        //UIColor *separatorColor = [UIColor colorWithRed:238.0f/255.0f green:238.0f/255.0f blue:238.0f/255.0f alpha:1.0f];
        
//        UIView *topLine = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.frame.size.width, kRowLineHeight)];
//        topLine.backgroundColor = separatorColor;
//        [_actionSheetView addSubview:topLine];
        ...
        if (title && title.length > 0)
        {
            ...
            CGFloat titleHeight = ceil([title boundingRectWithSize:CGSizeMake(self.frame.size.width-SPACE_TITLE_LEFT*2, MAXFLOAT) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName:[UIFont systemFontOfSize:kTitleFontSize]} context:nil].size.height) + SPACE_TITLE_TOP*2;
            
            UILabel *titleLabel = [[UILabel alloc] initWithFrame:CGRectMake(SPACE_TITLE_LEFT, actionSheetHeight, self.frame.size.width-SPACE_TITLE_LEFT*2, titleHeight)];
            ...
            UIView *titleBgView = [[UIView alloc] initWithFrame:CGRectMake(0, titleLabel.frame.origin.y, self.frame.size.width, titleLabel.frame.size.height)];
            titleBgView.backgroundColor = [UIColor colorWithRed:255.0f/255.0f green:255.0f/255.0f blue:255.0f/255.0f alpha:1.0f];
            [_actionSheetView addSubview:titleBgView];
        ...
        {
//            UIView *topLine = [[UIView alloc] initWithFrame:CGRectMake(0, actionSheetHeight, self.frame.size.width, kRowLineHeight)];
//            topLine.backgroundColor = separatorColor;
//            [_actionSheetView addSubview:topLine];
            actionSheetHeight += kRowLineHeight;
...
//                [_actionSheetView addSubview:topLine];
        ...
        if (cancelButtonTitle && cancelButtonTitle.length > 0)
        {
            ...
            effectView.frame = CGRectMake(0, actionSheetHeight, self.frame.size.width, kSeparatorHeight);
            [_actionSheetView addSubview:effectView];
