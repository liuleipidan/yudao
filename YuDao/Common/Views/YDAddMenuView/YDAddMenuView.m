...
- (id)init
{
    if (self = [super init]) {
        ...
        self.data = @[[YDAddMenuItem createWithType:YDAddMneuTypeFilter title:@"筛选" iconPath:@"dis_rank_filter"],
                      [YDAddMenuItem createWithType:YDAddMneuTypeShare title:@"分享" iconPath:@"dis_rank_share"]];
    }
    return self;
}
...
{
    YDAddMenuItem *item = [self.data objectAtIndex:indexPath.row];
    YDPopDownCell *cell = [tableView dequeueReusableCellWithIdentifier:@"YDPopDownCell"];
    [cell setItem:item];
    if (indexPath.row == self.data.count - 1) {
        [cell setHideSeperatorLine:YES];
    }
    return cell;
}
...
- (void)drawRect:(CGRect)rect
{
    ...
    [[UIColor blackTextColor] setFill];
    [[UIColor blackTextColor] setStroke];
...
- (UITableView *)tableView
{
    if (_tableView == nil) {
        ...
        [_tableView setBackgroundColor:[UIColor blackTextColor]];
