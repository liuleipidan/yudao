...
+ (id)actionSheetWithTouchItemBlock:(AWTouchItemBlock )touchItemBlock{
    AWActionSheet *sheet = [[AWActionSheet alloc] initWithIconSheetDelegate:nil ItemCount:5];
    [sheet setDidTouchItemBlock:touchItemBlock];
    return sheet;
}
...
- (void)DidTapOnItemAtIndex:(NSInteger)index title:(NSString*)name{
    YDClickSharePlatformType type = 0;
    if ([name isEqualToString:@"微信好友"]) {
        type = YDClickSharePlatformTypeWechat;
    }else if ([name isEqualToString:@"朋友圈"]){
        type = YDClickSharePlatformTypeWechatFriend;
    }else if ([name isEqualToString:@"QQ好友"]){
        type = YDClickSharePlatformTypeQQFriend;
    }else if ([name isEqualToString:@"QQ空间"]){
        type = YDClickSharePlatformTypeQQZone;
    }else{
        type = YDClickSharePlatformTypeWeibo;
    }
    if (self.didTouchItemBlock) {
        self.didTouchItemBlock(type);
    }
}
...
- (NSArray *)shareArray{
    if (_shareArray == nil) {
        ...
        if ([YDShareManager isInstalledPlatformType:YDThirdPlatformTypeWechat]) {
            [tempArr addObjectsFromArray:@[@{@"imageName":@"dynamic_share_1", @"title":@"微信好友"},
                                           @{@"imageName":@"dynamic_share_2", @"title":@"朋友圈"}]];
        }
        if ([YDShareManager isInstalledPlatformType:YDThirdPlatformTypeQQ]) {
            [tempArr addObjectsFromArray:@[@{@"imageName":@"dynamic_share_3", @"title":@"QQ好友"},
                                           @{@"imageName":@"dynamic_share_4", @"title":@"QQ空间"}]];
        }
        [tempArr addObject:@{@"imageName":@"dynamic_share_5", @"title":@"新浪微博"}];
        _shareArray = [NSArray arrayWithArray:tempArr];
    }
    return _shareArray;
}
...
- (void)actionForItem:(UITapGestureRecognizer*)recongizer
{
    ...
    [self dismiss];
    if (IconDelegate && [IconDelegate respondsToSelector:@selector(DidTapOnItemAtIndex:title:)]) {
