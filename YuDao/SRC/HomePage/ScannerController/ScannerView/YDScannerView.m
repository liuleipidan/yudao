...
- (void)startScanner
{
    ...
    timer = [NSTimer scheduledTimerWithTimeInterval:1.0 / 60 block:^(NSTimer *timer) {
        ...
        self.scannerLine.width = self.bgView.width * 1;
...
- (UIView *)bgView
{
    if (_bgView == nil) {
        ...
        [_bgView.layer setBorderColor:[UIColor clearColor].CGColor];
    }
    return _bgView;
}
...
{
    if (_scannerLine == nil) {
        _scannerLine = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"scanner_code_line"]];
        _scannerLine.contentMode = UIViewContentModeScaleAspectFit;
